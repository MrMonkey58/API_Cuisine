...
def modifier_quantite_ingredients(quantites, nombreConvives, nombreInitialRecette):
    ...
    for i in range(len(quantites)):
        ...
        modified_quantite = str(float(numbers[i]/nombreInitialRecette * nombreConvives)) + modified_quantite
...
def fct_recipe():
    ...
    if request.method == "GET":
        ...
        if recipe:
            # données récupérées depuis la table quantite_ingredient via la relation de clé étrangère.
            quantites_recette = [q.to_dict() for q in recipe.quantites]

            # Récupération des IDs des ingrédients qui sont contenus dans le dictionnaire quantites_recette
            ingredient_ids = [q['ingredient_id'] for q in quantites_recette]

            # Récupération les noms des ingrédients à partir de la table Ingredient en utilisant les ID
            ingredients = [Ingredient.query.get(ingredient_id).nom for ingredient_id in ingredient_ids]

            # Modification des quantités en fonction du nombre de convives (fonction créée plus haut)
            quantites_modifiees = modifier_quantite_ingredients([q['quantite'] for q in quantites_recette],
                                                                nombre_convives,
                                                                recipe.nombre_convives_de_base)

            # Création d'un dictionnaire contenant pour chaque ingrédient sa quantité modifiée
            dict_ingredients = {ingredient: quantite_modifiee for ingredient, quantite_modifiee in
                                zip(ingredients, quantites_modifiees)}
...
def fct_menus():
    ...
    if request.method == "DELETE":
        ...
        if menu and user:
            ...
            db.session.commit()
            return "Le menu " + nom + " a été supprimé de la liste de l'utilisateur."
        else:
            return "[ERREUR] : Les informations fournies sont incorrectes."

    else:
        # Retour méthodes prisent en charges
        return "[ERREUR] : Mauvaise méthode, POST, GET et DELETE acceptés."
...
@views.route("/delete-one-menu", methods=["DELETE"])
def delete_one_menu():
    if request.method == "DELETE":
        req_json = request.json
        nom = req_json['Nom_menu'].upper()
        ...
        if menu :
            ...
            db.session.commit()
            return "Le menu " + nom + " a été supprimé de la base de données."
        else:
            return "[ERREUR] : Les informations fournies sont incorrectes."

    else:
        # Retour méthodes prisent en charges
        return "[ERREUR] : Mauvaise méthode, DELETE acceptés."
